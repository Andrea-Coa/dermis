...
const FR_cnn: React.FC = () => {
  ...
  // Imágenes de ejemplo para el carrusel (laterales)
  const exampleImages = [
  ...
  const sendImageToAPI = async (): Promise<SkinAnalysisResult | null> => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.1.81:5000/api/analyze-skin/cnn', {
        method: 'POST',
        body: formData
      });
  ...
  return (
    <ScrollView contentContainerStyle={styles.container}>
      ...
      <Text style={styles.subtitle}>Ejemplos laterales</Text>
